...
class BudgetApiKtTest : ServerTest() {
    ...
    @Test
    fun testAuthorFiltering() {
        val author = createAuthor("Maga Salam")
        addRecord(BudgetRecord(2020, 5, 10, BudgetType.Приход, authorId = author.id))
        addRecord(BudgetRecord(2020, 5, 5, BudgetType.Приход, authorId = author.id))

        RestAssured.given()
            .queryParam("authorFio", "maga salam")
            .get("/budget/year/2020/stats")
            .toResponse<BudgetYearStatsResponse>().let { response ->
                println(response.items)

                Assert.assertEquals(2, response.items.size)
                Assert.assertEquals("Maga Salam", response.items[0].author?.fio)
                Assert.assertEquals("Maga Salam", response.items[1].author?.fio)
            }
    }
    ...
    private fun createAuthor(fio: String): AuthorEntity {
        return AuthorEntity.new { this.fio = fio }
    }
}